...
namespace FastDirTest
{
    ...
    public class FastFileInfo
    {
        ...
        public static IEnumerable<FastFileInfo> EnumerateDirectories(string path) => EnumerateDirectories(path, "*");
        public static IEnumerable<FastFileInfo> EnumerateDirectories(string path, string searchPattern) => EnumerateDirectories(path, searchPattern, SearchOption.TopDirectoryOnly);
        public static IEnumerable<FastFileInfo> EnumerateDirectories(string path, string searchPattern, SearchOption searchOption) => EnumerateDirectories(path, searchPattern, searchOption, null);
        public static IEnumerable<FastFileInfo> EnumerateDirectories(string path, string searchPattern, SearchOption searchOption, IFolderFilter folderFilter)
        {
        ...
        public static IEnumerable<FastFileInfo> EnumerateFiles(string path) => EnumerateFiles(path, "*");
        public static IEnumerable<FastFileInfo> EnumerateFiles(string path, string searchPattern) => EnumerateFiles(path, searchPattern, SearchOption.TopDirectoryOnly, null);
        public static IEnumerable<FastFileInfo> EnumerateFiles(string path, string searchPattern, SearchOption searchOption) => EnumerateFiles(path, searchPattern, searchOption, null);
        public static IEnumerable<FastFileInfo> EnumerateFiles(string path, string searchPattern, SearchOption searchOption, IFolderFilter folderFilter)
        {
            ExceptionHandle(path, searchPattern, searchOption);
        ...
        public static IList<FastFileInfo> GetFiles(string path, string searchPattern = "*", SearchOption searchOption = SearchOption.TopDirectoryOnly, IFolderFilter folderFilter = null)
        {
            var list = new List<FastFileInfo>();
        ...
        private class FileEnumerable(string path, string filter, SearchOption searchOption, IFolderFilter folderFilter, bool searchForFiles) : IEnumerable<FastFileInfo>
        {
            public IEnumerator<FastFileInfo> GetEnumerator() => new FileEnumerator(path, filter, searchOption, folderFilter, searchForFiles, true, false, false);
        ...
        public class FileEnumerator : IEnumerator<FastFileInfo>
        {
            ...
            public FastFileInfo Current => new(currentFolder, findData);
            ...
            object IEnumerator.Current => new FastFileInfo(currentFolder, findData);

            public bool MoveNext()
            {
                // Loop until a file or directory is found
                while (true)
                {
                    ...
                    if (isCurrent || !stepToNext)
                    {
                        while (findData.dwFileAttributes.HasFlag(FileAttributes.Directory))
                        {
                            ...
                            {
                                if (folderFilter == null || folderFilter.SearchFolder(new FastFileInfo(currentFolder, findData)))
                                    currentPaths.Add(Path.Combine(currentFolder, cFilename));
                                if (searchScope == 1)
                                {
                                    stepToNext = true;
                                    return true;
                    ...
                    // Handle pending folders and update current path, at this point, the current folder is exhausted.
                    // If search subfolders then enqueue them.
                    ...
                    // Update the current index and path, or finish if done
                    currentPathIndex++;
